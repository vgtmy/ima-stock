...
from config import (
    BASE_DIR, DATA_DIR, OUTPUT_DIR, MAX_RETRIES, RETRY_DELAY,
...
def fetch_all_data(target_date: str = None, use_cache: bool = True) -> dict:
    ...
    # 检查多个可能位置（兼容用户实际存放路径）
    snapshot_search_paths = [
        os.path.join(DATA_DIR, snapshot_file),       # data/ 目录
        os.path.join(BASE_DIR, snapshot_file),       # 项目根目录
        snapshot_file,                               # 当前工作目录
    ]
    for snapshot_path in snapshot_search_paths:
        if os.path.exists(snapshot_path):
            try:
                with open(snapshot_path, "rb") as f:
                    raw_data = pickle.load(f)
                logger.info(f"📦 从快照恢复原始数据: {snapshot_path} ({len(raw_data.get('stock_list', []))} 只股票)")
                return raw_data
            except Exception as e:
    ...
    try:
        # 同时存到 BASE_DIR（项目根）和 DATA_DIR（向后兼容）
        for save_dir in [BASE_DIR, DATA_DIR]:
            try:
                sp = os.path.join(save_dir, f"raw_data_{target_date}.pkl")
                with open(sp, "wb") as f:
                    pickle.dump(raw_data, f)
                logger.info(f"📦 原始数据快照已保存: {sp} ({len(stock_list)} 只股票)")
            except Exception as e:
                logger.warning(f"⚠️ 快照保存到 {save_dir} 失败: {e}")
    except Exception as e:
