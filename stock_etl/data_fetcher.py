...
def _run_westock(subcmd: str, args: str = "", timeout: int = 60) -> str:
    """执行 westock-data CLI 命令，返回原始 stdout"""
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            # 检查是否返回了错误 JSON
            if result.stdout.strip().startswith("{") and '"success": false' in result.stdout:
                logger.debug(f"  [{subcmd}] API 返回错误: {result.stdout[:200]}")
            raise RuntimeError(f"westock {subcmd} 返回空或失败")
...
def fetch_stock_list(use_cache: bool = True) -> pd.DataFrame:
    ...
    logger.info("[股票列表] 从 akshare 获取代码列表...")
    try:
        ...
        logger.warning(f"[股票列表] akshare 获取失败: {e}, 使用 westock search 替代")
        # fallback: 使用实时行情获取
        try:
            text = _run_westock("hot", "stock")
            df_hot = _parse_markdown_table(text)
    ...
    logger.info(f"[股票列表] ✅ 共 {len(df)} 只A股")
